...
encrypted_user_marketing_info = pd.DataFrame(columns = user_marketing_info.columns)

def encrypt_marketing_data(row: pd.Series):
    global encrypted_user_marketing_info
    ...
    for col in row.index:
        ...
        full_policy = ' or '.join(row_policy + [user_marketing_access_policy[col]])
    ...
    encrypted_user_marketing_info = pd.concat([encrypted_user_marketing_info, pd.DataFrame(encrypted_row)], axis=0, ignore_index=True)


# Apply it on the first 5 rows of User Marketing Info
user_marketing_info.head().apply(encrypt_marketing_data, axis=1)
